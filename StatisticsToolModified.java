...
package pt.ulisboa.tecnico.instrumentation;
...
public class StatisticsToolModified
{
			...
			for (int i = 0; i < filelist.length; i++) {
				...
				if (filename.endsWith(".class")) {
					...
					for (Enumeration e = ci.getRoutines().elements(); e.hasMoreElements(); ) {
						Routine routine = (Routine) e.nextElement();
						routine.addBefore("StatisticsToolModified", "dynMethodCount", new Integer(1));
					...
					for (Enumeration e = ci.getRoutines().elements(); e.hasMoreElements(); ) {
						...
						for (Enumeration instrs = (routine.getInstructionArray()).elements(); instrs.hasMoreElements(); ) {
							...
							if (opcode == InstructionTable.getfield)
								instr.addBefore("StatisticsToolModified", "LSFieldCount", new Integer(0));
							else if (opcode == InstructionTable.putfield)
								instr.addBefore("StatisticsToolModified", "LSFieldCount", new Integer(1));
							else {
								short instr_type = InstructionTable.InstructionTypeTable[opcode];
								if (instr_type == InstructionTable.LOAD_INSTRUCTION) {
									instr.addBefore("StatisticsToolModified", "LSCount", new Integer(0));
								}
								else if (instr_type == InstructionTable.STORE_INSTRUCTION) {
									instr.addBefore("StatisticsToolModified", "LSCount", new Integer(1));
								}
							}
						}
	...
	public static void doBranch(File in_dir, File out_dir) 
		{
			...
			for (int i = 0; i < filelist.length; i++) {
				...
				if (filename.endsWith(".class")) {
					...
					for (Enumeration e = ci.getRoutines().elements(); e.hasMoreElements(); ) {
						Routine routine = (Routine) e.nextElement();
						routine.addBefore("StatisticsToolModified", "setBranchMethodName", routine.getMethodName());
						InstructionArray instructions = routine.getInstructionArray();
						for (Enumeration b = routine.getBasicBlocks().elements(); b.hasMoreElements(); ) {
							BasicBlock bb = (BasicBlock) b.nextElement();
							Instruction instr = (Instruction) instructions.elementAt(bb.getEndAddress());
							short instr_type = InstructionTable.InstructionTypeTable[instr.getOpcode()];
							if (instr_type == InstructionTable.CONDITIONAL_INSTRUCTION) {
								instr.addBefore("StatisticsToolModified", "setBranchPC", new Integer(instr.getOffset()));
								instr.addBefore("StatisticsToolModified", "updateBranchNumber", new Integer(k));
								instr.addBefore("StatisticsToolModified", "updateBranchOutcome", "BranchOutcome");
								k++;
							}
						}
